...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE residences ADD COLUMN IF NOT EXISTS plan VARCHAR DEFAULT 'free'")
    ...
    op.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS residence_id INTEGER")
    ...
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS residence_id INTEGER")
    op.execute("UPDATE residents SET residence_id = 1 WHERE residence_id IS NULL")

    # 4. Missing Cleanup Columns (JSONB, etc)
    # hospitalization_history (JSONB)
    op.execute(
        "ALTER TABLE residents ADD COLUMN IF NOT EXISTS "
        "hospitalization_history JSONB DEFAULT '[]'::jsonb"
    )
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS hospitalization_end_date DATE")
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS hospitalization_hospital VARCHAR")
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS hospitalization_reason VARCHAR")

    # wounds (JSONB)
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS wounds JSONB DEFAULT '[]'::jsonb")
    ...
    # sexuality
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS sexuality_observations TEXT")

    # sleep
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS sleep_medication VARCHAR")

    # ensure others
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS mobility_level VARCHAR")
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS cognitive_impairment VARCHAR")
    op.execute("ALTER TABLE residents ADD COLUMN IF NOT EXISTS additional_data JSONB")
